...
fn setup(initial_balance: i128, platform_fee_bps: u32) -> TestFixture {
    ...
    let token = deploy_token_contract(&env, &token_admin);
...
fn test_lock_with_3_tranches_and_release_one_by_one() {
    ...
    f.client.lock_with_tranches(&f.id, &f.seller, &f.buyer, &500, &tranches, &100).unwrap();
    ...
    // Release tranche 1 (150 - 1% = 148, fee rounds down to 1)
    let payout2 = f.client.release_tranche(&f.id, &1, &secret2);
    assert_eq!(payout2, 148);
    assert_eq!(f.token.balance(&f.seller), 346);
    ...
    // Release tranche 2 (150 - 1% = 148)
    let payout3 = f.client.release_tranche(&f.id, &2, &secret3);
    assert_eq!(payout3, 148);
    assert_eq!(f.token.balance(&f.seller), 494);
    assert_eq!(f.token.balance(&f.admin), 6);
...
fn test_tranche_sum_mismatch_rejected() {
    ...
    let result = f.client.try_lock_with_tranches(&f.id, &f.seller, &f.buyer, &500, &tranches, &100);
...
fn test_tranche_accounting_invariant() {
    ...
    f.client.lock_with_tranches(&f.id, &f.seller, &f.buyer, &total_locked, &tranches, &100).unwrap();
    ...
    assert_eq!(total_paid_to_seller + refunded_to_buyer + total_fees, total_locked);
    
    // Verify actual balances
    assert_eq!(f.token.balance(&f.seller), total_paid_to_seller);
    assert_eq!(f.token.balance(&f.buyer), 2_000 - total_locked + refunded_to_buyer);
