...
use super::*;
use soroban_sdk::{testutils::{Address as _, Ledger as _}, token, Address, BytesN, Env, Vec};
...
fn test_lock_with_3_tranches_and_release_one_by_one() {
    ...
    f.client.lock_with_tranches(&f.id, &f.seller, &f.buyer, &500, &tranches, &100);
...
fn test_tranche_accounting_invariant() {
    ...
    f.client.lock_with_tranches(&f.id, &f.seller, &f.buyer, &total_locked, &tranches, &100);
