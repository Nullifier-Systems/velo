/**
 * State Space Model Checker for Step 3a: Message Delay / Network Asynchrony
 */
...
function getNextStates(s) {
  ...
  // RelayerAttest (Relayer attestations subject to network delay)
  ...
  // Tick (Network delay allows time to tick independently of pending relayers)
  if (s.time < MaxTime) {
    next.push({
      action: "Tick",
...
function modelCheck() {
  ...
  console.log("=== TLC Model Checker Execution Output (Step 3a: Network Delay) ===");
  console.log(`Specification: formal/htlc-atomicity/HTLCAtomicityNetworkDelay.tla`);
  console.log(`Configuration: Threshold=${Threshold}, TotalRelayers=${TotalRelayers}, MaxTime=${MaxTime}, evmTimeout=10, stellarTimeout=5`);
