...
```
*Note: This only works if all dependencies strictly allow `^0.6.4`. If an upstream crate explicitly pinned an incompatible version, this will fail.*
...
rand_core = "=0.6.4"
```
*(You may need to adjust the version strings based on the exact cargo tree conflict shown by `cargo tree -d`).*
