  ...
  app.get<{ Params: { providerId: string; documentId: string } }>(
    ...
    async (req, reply) => {
      if ((app as any).pg) {
        ...
        if (!rows[0]) return throw new ApiError(404, "DOCUMENT_NOT_FOUND", "Verification document not found");const fileName = String(rows[0].file_name).replace(/[\"\r\n]/g, "_");
      ...
      if (!document || document.providerId !== req.params.providerId) {
        return throw new ApiError(404, "DOCUMENT_NOT_FOUND", "Verification document not found");}
  ...
  app.post<{ Params: { id: string }; Body: { status?: string } }>(
    ...
    async (req, reply) => {
      ...
      if (status !== "approved" && status !== "rejected") {
        return throw new ApiError(400, "INVALID_PARAMETER", "status must be 'approved' or 'rejected'");}
      ...
      if ((app as any).pg) {
        if (status === "approved") {
          ...
          if (!documents.rows[0]) return throw new ApiError(409, "DOCUMENT_REQUIRED", "A submitted verification document is required before approval");}
        const { rows } = await (app as any).pg.query(
        ...
        if (!rows[0]) return throw new ApiError(404, "PROVIDER_NOT_FOUND", "Provider not found");} else {
        if (!getProviderById(req.params.id)) return throw new ApiError(404, "PROVIDER_NOT_FOUND", "Provider not found");if (status === "approved" && getProviderVerificationDocuments(req.params.id).length === 0) {
          return throw new ApiError(409, "DOCUMENT_REQUIRED", "A submitted verification document is required before approval");}
      }
