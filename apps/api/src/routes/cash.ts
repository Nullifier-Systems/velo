...
// Keep main's new prepare endpoint here
app.post<{ Body: z.infer<typeof prepareLockSchema> }>(
  ...
  async (req, reply) => {
    // main implementation stays unchanged
  },
);

// Keep feature branch existing request endpoint
app.post<{ Body: CashRequestBody; Querystring: { lang?: string } }>(
  "/cash/request",
  async (req, reply) => {
    // existing claim-ticket-a11y implementation stays here
  },
);
    "/cash/request",
    {
