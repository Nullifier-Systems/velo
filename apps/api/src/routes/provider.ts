...
const registerProviderSchema = z.object({
  stellar_address: z
    .string()
    .trim()
    .regex(/^G[1-9A-HJ-NP-Za-km-z]{55}$/, 'Invalid Stellar address format'),
...
async function handleProviderRegistration(
  req: FastifyRequest,
  reply: FastifyReply,
  app: FastifyInstance
) {
...
export async function providerRoutes(app: FastifyInstance) {
  ...
  app.post('/providers/register', async (req, reply) =>
    handleProviderRegistration(req, reply, app)
  );
  ...
  app.get('/providers', async (req, reply) => {
    ...
    return reply.send({
      providers: records.map((p) => toPublicProvider(p, undefined, DEFAULT_PRECISION)),
    });
  });
  ...
  app.get('/provider/export', async (req, reply) => {
    ...
    if (format === 'csv') {
      ...
      reply
        ...
        .header(
          'Content-Disposition',
          `attachment; filename="completed_trades_${providerAddress.substring(0, 8)}.csv"`
        )
        .send(csvContent);
      return;
    }
    ...
    reply
      ...
      .header(
        'Content-Disposition',
        `attachment; filename="completed_trades_${providerAddress.substring(0, 8)}.json"`
      )
      .send(jsonOutput);
  });
}
