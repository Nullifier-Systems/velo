...
export const app = Fastify({
  ...
  genReqId: (req) => {
    const inbound =
      firstHeaderValue(req.headers['x-request-id']) ?? firstHeaderValue(req.headers['x-vercel-id']);
...
app.register(cors, {
  origin: ['http://localhost:5181', process.env.FRONTEND_BASE_URL ?? 'http://localhost:5181'],
});
